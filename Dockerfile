ARG BASE_IMAGE=mcr.microsoft.com/dotnet/core/sdk:3.1.202
FROM $BASE_IMAGE AS build
ARG CORECLR_BRANCH=v3.1.4
RUN apt-get update && \
	apt-get install -y \
		cmake \
		llvm-6.0 \
		clang-6.0 \
		lldb-6.0 \
		liblldb-6.0-dev \
		libunwind8 \
		libunwind8-dev \
		gettext \
		libicu-dev \
		liblttng-ust-dev \
		libcurl4-openssl-dev \
		libssl-dev \
		uuid-dev \
		libnuma-dev \
		libkrb5-dev \
		git && \
	git clone https://github.com/dotnet/coreclr.git /coreclr
WORKDIR /coreclr
RUN git checkout $CORECLR_BRANCH
COPY patches /patches
RUN if [ -f /patches/$CORECLR_BRANCH.patch ] ; then git apply /patches/$CORECLR_BRANCH.patch ; fi
RUN ./build.sh clang6.0 

FROM $BASE_IMAGE
RUN apt-get update && \
	apt-get install -y \
		lldb-6.0 && \
	rm -rf /var/lib/apt/lists/* && \
	ln -s /coreclr/createdump /usr/bin/createdump
COPY --from=build /coreclr/bin/Product/Linux.x64.Debug /coreclr
RUN dotnet tool install -g dotnet-sos

ENV PATH="$PATH:/root/.dotnet/tools"
RUN dotnet-sos install
ENV COREDUMP_PATH /tmp/coredump
CMD /usr/bin/lldb-6.0 /usr/bin/dotnet --core $COREDUMP_PATH
